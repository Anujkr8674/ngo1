...
interface AgmClientProps {
  reports: AgmReport[]
}

export default function AgmClient({ reports }: AgmClientProps) {
  const [activePdf, setActivePdf] = useState<AgmReport | null>(
  ...
  if (!reports || reports.length === 0) {
    return (
      <div className="flex flex-col w-full min-h-screen">
        <section className="relative py-24 px-6 md:px-12 flex items-center justify-center border-b border-foreground/5 min-h-[90vh]">
          ...
          <div className="max-w-4xl mx-auto w-full z-10 relative text-center">
            ...
            <p className="text-white/80 mt-4">No reports available at the moment. Please check back later.</p>
          </div>
        </section>
      </div>
    );
        ...
        <div className="bg-[#E5DDF8] rounded-[3rem] p-4 sm:p-8 md:p-12 border border-foreground/5 max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 items-start min-h-[80vh]">
          ...
          <div className="w-full lg:w-1/3 flex flex-col gap-4">
            ...
            {Math.ceil(reports.length / ITEMS_PER_PAGE) > 1 && (
              <div className="flex items-center justify-center gap-1.5 mt-4 pt-3 border-t border-[#C8B4D4]/30">
                <button
                  disabled={currentPage === 1}
                  onClick={() => setCurrentPage(currentPage - 1)}
                  className="px-3 py-1.5 text-xs font-bold rounded-xl border border-[#C8B4D4]/50 bg-white/60 hover:bg-white text-foreground/75 disabled:opacity-40 transition-all cursor-pointer"
                >
                  Prev
                </button>
                
                {Array.from({ length: Math.ceil(reports.length / ITEMS_PER_PAGE) }).map((_, idx) => {
                  const pageNum = idx + 1
                  return (
                    <button
                      key={pageNum}
                      ...
                      className={`px-3 py-1.5 text-xs font-bold rounded-xl border transition-all cursor-pointer ${
                        currentPage === pageNum
                          ? 'bg-white border-[#C8B4D4] text-foreground shadow-sm'
                ...
                <button
                  disabled={currentPage === Math.ceil(reports.length / ITEMS_PER_PAGE)}
                  onClick={() => setCurrentPage(currentPage + 1)}
                  className="px-3 py-1.5 text-xs font-bold rounded-xl border border-[#C8B4D4]/50 bg-white/60 hover:bg-white text-foreground/75 disabled:opacity-40 transition-all cursor-pointer"
                >
                  Next
