...
import { deleteElderlySupport } from '@/app/actions/getHelp'
...
export default function ElderlyAdminClient({ initialRecords }: { initialRecords: any[] }) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this elderly support request?')) return
    setDeletingId(id)
    try {
      const res = await deleteElderlySupport(id)
      if (res.success) {
  ...
  const filtered = records.filter(r => {
    ...
    return (
      ...
      r.livingStatus?.toLowerCase().includes(term) ||
      r.emergencyContact?.toLowerCase().includes(term) ||
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 p-6 md:p-8 shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Elderly Support Submissions</h2>
          <p className="text-xs text-slate-400 mt-1">Manage elder care, healthcare, and daily living support requests.</p>
        </div>
        <div className="relative max-w-md w-full">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input
            type="text"
            placeholder="Search by elder name, mobile, living status..."
            value={searchTerm}
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="border-b border-slate-100 text-slate-400 font-semibold uppercase tracking-wider font-sans">
              ...
              <th className="py-4 px-4">Living Status</th>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50/55 transition-colors">
                  ...
                  <td className="py-4 px-4 text-slate-600 font-semibold font-sans">{record.age}</td>
                  <td className="py-4 px-4 text-slate-500">
                    <span className="inline-flex px-2 py-0.5 rounded-full font-bold text-[10px] bg-purple-50 text-purple-700 border border-purple-100">
                      {record.livingStatus}
                    </span>
                  </td>
                  <td className="py-4 px-4 text-slate-650">{record.emergencyContact} ({record.emergencyPhone})</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl border border-slate-100 animate-scaleUp">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <span className="inline-flex px-2 py-0.5 rounded-full font-bold text-[10px] mb-1.5 bg-green-50 text-green-700 border border-green-100">
                  Elderly Support Request
                </span>
                <h3 className="text-lg font-bold text-slate-800">Elderly Person Details</h3>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-sm text-slate-600">
              ...
              {selectedRecord.photoUrl && (
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  />
                  <div>
                    <h4 className="text-slate-800 font-bold">Elderly Passport Photo</h4>
              ...
              {/* Personal Details */}
              <div>
                <h4 className="font-bold text-xs uppercase tracking-wider text-purple-700 border-b border-purple-50 pb-1.5 mb-3 font-sans">Personal Information</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Elderly Name</h5>
                    <p className="font-bold text-slate-800 mt-0.5">{selectedRecord.name}</p>
                  </div>
                  <div>
                    <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Age</h5>
                    <p className="font-semibold text-slate-700 mt-0.5">{selectedRecord.age}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Living Status</h5>
                    <p className="mt-0.5 font-bold text-purple-750">{selectedRecord.livingStatus}</p>
                  </div>
                  <div>
                ...
                <div className="mt-3">
                  <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Living Address</h5>
                  <p className="mt-0.5 text-slate-700 font-medium">{selectedRecord.address}</p>
                </div>
              </div>

              {/* Emergency Contact */}
              <div>
                <h4 className="font-bold text-xs uppercase tracking-wider text-red-700 border-b border-red-50 pb-1.5 mb-3 font-sans">Emergency Contact Information</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Emergency Contact Person</h5>
                    <p className="font-bold text-slate-800 mt-0.5">{selectedRecord.emergencyContact}</p>
                  </div>
                  <div>
                    <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Emergency Phone</h5>
                    <p className="font-bold text-slate-800 mt-0.5">{selectedRecord.emergencyPhone}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Support & Health */}
              <div>
                <h4 className="font-bold text-xs uppercase tracking-wider text-blue-700 border-b border-blue-50 pb-1.5 mb-3 font-sans">Health & Support Requirements</h4>
                <div className="space-y-3">
                  {selectedRecord.healthStatus && (
                    <div>
                      <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Health Status Description</h5>
                      <p className="mt-0.5 font-medium text-slate-700">{selectedRecord.healthStatus}</p>
                    </div>
                  )}
                  <div>
                    <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider mb-1">Services Needed</h5>
                  ...
                  {selectedRecord.otherSupport && (
                    <div>
                      <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Other Requirements</h5>
                  ...
                  {selectedRecord.suggestions && (
                    <div>
                      <h5 className="text-slate-400 text-[10px] font-semibold uppercase tracking-wider">Special Requests / Suggestions</h5>
