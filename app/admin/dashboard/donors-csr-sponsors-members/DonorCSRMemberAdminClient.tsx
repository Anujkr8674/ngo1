...
import { 
  createDonorCSRMember, 
  updateDonorCSRMember, 
  deleteDonorCSRMember, 
  reorderDonorCSRMembers 
} from '@/app/actions/donorCSRMember'
import { 
  Plus, 
  Edit3, 
  Trash2, 
...
interface DonorCSRMemberAdminClientProps {
  initialMembers: DonorCSRMember[]
}

const ITEMS_PER_PAGE = 10

export default function DonorCSRMemberAdminClient({ initialMembers }: DonorCSRMemberAdminClientProps) {
  const [members, setMembers] = useState<DonorCSRMember[]>(initialMembers)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editingMember, setEditingMember] = useState<DonorCSRMember | null>(null)
  ...
  // Open modal to Edit
  const handleOpenEdit = (member: DonorCSRMember) => {
    setEditingMember(member)
    setTitle(member.title)
    setImageFile(null)
    setImageFileName('')
    setImagePreviewUrl(member.src)
    setMessage(null)
    setIsModalOpen(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMember) {
        const res = await updateDonorCSRMember(editingMember.id, formData)
        if (res.error) {
          setMessage({ type: 'error', text: res.error })
        } else if (res.member) {
          setMembers(members.map(m => m.id === editingMember.id ? (res.member as DonorCSRMember) : m))
          setMessage({ type: 'success', text: 'Record updated successfully!' })
          setTimeout(() => setIsModalOpen(false), 1500)
        }
      } else {
        const res = await createDonorCSRMember(formData)
        if (res.error) {
          setMessage({ type: 'error', text: res.error })
        } else if (res.member) {
          setMembers([...members, res.member as DonorCSRMember]) // Adds at the end
          setCurrentPage(Math.ceil((members.length + 1) / ITEMS_PER_PAGE)) // Return to last page to see it
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this sheet/record? This will permanently delete the file from storage.')) {
      return
    }

    setLoading(true)
    try {
      const res = await deleteDonorCSRMember(id)
      if (res.success) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-slate-100">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Donors & CSR Sponsors</h1>
          <p className="text-slate-500 text-sm mt-1">Upload and manage patron registries and sponsor display sheets.</p>
        </div>
        <button
          onClick={handleOpenAdd}
          className="inline-flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:shadow transition-all cursor-pointer"
        >
      ...
      <div className="mt-6 flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="w-full md:max-w-md relative">
          <input
            ...
            className="w-full pl-4 pr-10 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 text-sm"
          />
          {searchQuery && (
            <button 
            ...
            {paginatedMembers.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-slate-400">
                  No sheets found. Click "Add Sheet / Image" to upload.
                </td>
              </tr>
            ) : (
              paginatedMembers.map((doc, idx) => {
                const absoluteIndex = (currentPage - 1) * ITEMS_PER_PAGE + idx
                const isFirst = absoluteIndex === 0
                const isLast = absoluteIndex === filteredMembers.length - 1
                  ...
                  <tr key={doc.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div 
                        ...
                        className="w-16 aspect-[4/5] rounded-lg overflow-hidden border border-slate-200 cursor-zoom-in relative group/thumb"
                      >
                        <img 
                          src={doc.src} 
                          alt={doc.title} 
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg border border-slate-100 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100">
              <h2 className="text-lg font-black text-slate-800">
                {editingMember ? 'Edit Sponsor Details' : 'Upload Sponsor Sheet'}
              </h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  placeholder="e.g. Patron Donors & Life Members - Sheet 4"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  disabled={loading}
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 text-sm font-semibold"
                  required
                />
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 block">
                  Sponsor Image File {editingMember && <span className="text-[10px] text-slate-400 font-normal lowercase">(optional: select file to replace)</span>}
                </label>
                
                <div className="relative border-2 border-dashed border-slate-200 hover:border-orange-500/50 rounded-2xl p-6 transition-colors flex flex-col items-center justify-center bg-slate-50/50">
                  <input
                    type="file"
                    accept="image/*"
                ...
                {editingMember && !imageFile && (
                  <div className="text-[11px] text-slate-500 font-bold mt-1 bg-slate-100 px-3 py-1 rounded-md inline-block">
                    Current Image URL: <a href={editingMember.src} target="_blank" rel="noopener noreferrer" className="text-orange-650 hover:underline">Link</a>
                  </div>
                )}
                ...
                {imagePreviewUrl && (
                  <div className="mt-3 flex flex-col items-center">
                    ...
                    <div 
                      ...
                      className="w-32 aspect-[4/5] rounded-xl overflow-hidden border border-slate-200 cursor-zoom-in hover:opacity-90 relative group/preview"
                    >
                      <img 
                        src={imagePreviewUrl} 
                        alt="Preview" 
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-600/50 text-white font-bold py-2.5 rounded-xl text-sm transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
                >
                  {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                  {editingMember ? 'Save Changes' : 'Upload & Add'}
                </button>
              </div>
            </form>
          </div>
