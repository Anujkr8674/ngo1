...
interface TestimonialAdminClientProps {
  initialTestimonials: Testimonial[]
}

const ITEMS_PER_PAGE = 10

export default function TestimonialAdminClient({ initialTestimonials }: TestimonialAdminClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingTestimonial && (!videoFile || videoFile.size === 0)) {
    ...
    try {
      if (editingTestimonial) {
        ...
        if (res.error) {
          ...
          setTestimonials(testimonials.map(t => t.id === editingTestimonial.id ? (res.testimonial as Testimonial) : t))
        ...
        if (res.error) {
          ...
          setTestimonials([...testimonials, res.testimonial as Testimonial])
          setCurrentPage(Math.ceil((testimonials.length + 1) / ITEMS_PER_PAGE))
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this testimonial? This will permanently delete both the cover and the video file.')) {
      return
    }
  ...
  const paginatedTestimonials = filteredTestimonials.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE)
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-6 md:p-8 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-slate-100">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Video Testimonials</h1>
          <p className="text-slate-500 text-sm mt-1">Upload and manage NGO supporter video stories, descriptions, and cover screenshots.</p>
        </div>
        <button
      ...
      <div className="mt-6 flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="text-slate-400 text-xs font-semibold">
          Showing {filteredTestimonials.length} of {testimonials.length} records
        </div>
      </div>
        ...
        <table className="w-full border-collapse text-left text-sm text-slate-600">
          <thead className="bg-slate-50 text-slate-500 text-xs font-bold uppercase tracking-wider border-b border-slate-100">
            <tr>
              ...
              <th className="px-6 py-4 w-28">Cover Image</th>
              <th className="px-6 py-4">Supporter Name</th>
              <th className="px-6 py-4 w-40">Source Path</th>
            ...
            {paginatedTestimonials.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-slate-400">
                  No testimonials found. Click "Add Testimonial" to upload.
                </td>
              </tr>
            ) : (
              paginatedTestimonials.map((doc, idx) => {
                const absoluteIndex = (currentPage - 1) * ITEMS_PER_PAGE + idx
                const isFirst = absoluteIndex === 0
                const isLast = absoluteIndex === filteredTestimonials.length - 1
                const isSupabase = doc.video.startsWith('http')
                ...
                return (
                  <tr key={doc.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div 
                        onClick={() => {
                          setPreviewVideoUrl(doc.video)
                          setPreviewVideoTitle(doc.name)
                        }}
                        className="w-16 aspect-[4/3] rounded-lg overflow-hidden border border-slate-200 cursor-zoom-in relative group/thumb bg-slate-100"
                      >
                        <img 
                          src={doc.image} 
                          alt={doc.name} 
          ...
          <div className="bg-white rounded-3xl w-full max-w-xl border border-slate-100 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center px-6 py-5 border-b border-slate-100">
              <h2 className="text-lg font-black text-slate-800">
                {editingTestimonial ? 'Edit Testimonial Details' : 'Upload Video Testimonial'}
              </h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 block">
                  Video File (.mp4, .webm) {editingTestimonial && <span className="text-[10px] text-slate-400 font-normal lowercase">(optional: replace)</span>}
                </label>
                
                <div className="relative border-2 border-dashed border-slate-200 hover:border-purple-500/50 rounded-2xl p-5 transition-colors flex flex-col items-center justify-center bg-slate-50/50">
                  <input
                    type="file"
                    accept="video/*"
                  ...
                  <p className="text-xs text-slate-500 font-semibold text-center">
                    {videoFileName ? videoFileName : 'Drag and drop video here, or click to browse'}
                  </p>
                </div>
                {editingTestimonial && !videoFile && (
                  <div className="text-[11px] text-slate-500 font-bold mt-1 bg-slate-100 px-3 py-1 rounded-md inline-block">
                    Current Video: <a href={editingTestimonial.video} target="_blank" rel="noopener noreferrer" className="text-purple-650 hover:underline">Link</a>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-400 block">
                  Cover / Screenshot Image {editingTestimonial && <span className="text-[10px] text-slate-400 font-normal lowercase">(optional: replace)</span>}
                </label>
                ...
                <div className="relative border-2 border-dashed border-slate-200 hover:border-purple-500/50 rounded-2xl p-5 transition-colors flex flex-col items-center justify-center bg-slate-50/50">
                  ...
                  <p className="text-xs text-slate-500 font-semibold text-center">
                    {imageFileName ? imageFileName : 'Drag and drop cover image, or click to browse'}
                  </p>
                </div>

                {imagePreviewUrl && (
                  <div className="mt-3 flex flex-col items-center">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 self-start">Cover Image Preview</p>
                    <div className="w-32 aspect-[4/3] rounded-xl overflow-hidden border border-slate-200 bg-black">
                      <img 
                        src={imagePreviewUrl} 
                        alt="Preview" 
                        className="w-full h-full object-cover" 
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  ...
                  {editingTestimonial ? 'Save Changes' : 'Upload & Add'}
                </button>
              </div>
            </form>
          </div>
      ...
      {previewVideoUrl && (
        <div className="fixed inset-0 bg-black/90 z-[120] backdrop-blur-sm flex items-center justify-center p-4 sm:p-8" onClick={() => setPreviewVideoUrl(null)}>
          <div className="relative w-full max-w-4xl aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <button 
              onClick={() => {
                setPreviewVideoUrl(null)
                setPreviewVideoTitle('')
              }}
              className="absolute top-4 right-4 z-50 p-2 bg-black/60 hover:bg-black/80 text-white rounded-full transition-colors cursor-pointer shadow"
            >
              <X className="w-5 h-5" />
            </button>
            <video
