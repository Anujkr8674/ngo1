...
import { deleteVolunteer } from '@/app/actions/volunteer'
...
export default function VolunteerAdminClient({ initialRecords }: { initialRecords: any[] }) {
  ...
  const filtered = records.filter(r => {
    ...
    return (
      ...
      r.qualification?.toLowerCase().includes(term) ||
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 p-6 md:p-8 shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Volunteer Submissions</h2>
          <p className="text-xs text-slate-400 mt-1">Manage, view, and inspect registered volunteer details.</p>
        </div>
        <div className="relative max-w-md w-full">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input
            type="text"
            placeholder="Search by name, mobile, email, qualification..."
            value={searchTerm}
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="border-b border-slate-100 text-slate-400 font-semibold uppercase tracking-wider font-sans">
              ...
              <th className="py-4 px-4">Commitment</th>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50/55 transition-colors">
                  ...
                  <td className="py-4 px-4 text-slate-600">{record.gender}</td>
                  <td className="py-4 px-4 text-slate-600 font-medium">{record.mobile}</td>
                  <td className="py-4 px-4 text-slate-600 font-medium">{record.email}</td>
                  <td className="py-4 px-4 text-slate-500">{record.hoursPerWeek}</td>
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl border border-slate-100 animate-scaleUp">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <h3 className="text-lg font-bold text-slate-800">Volunteer Details</h3>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6 text-slate-700">
              <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-xs">
                ...
                <div>
                  <span className="block text-slate-400 font-semibold mb-0.5">Language Proficiency</span>
                  <span className="font-semibold text-slate-800">{selectedRecord.language}</span>
                </div>
                <div>
                  <span className="block text-slate-400 font-semibold mb-0.5">Education / Qualification</span>
                  <span className="font-semibold text-slate-800">{selectedRecord.qualification}</span>
                </div>
                <div>
                ...
                <div>
                  <span className="block text-slate-400 font-semibold mb-0.5">Commitment Time</span>
                  <span className="font-semibold text-slate-800">{selectedRecord.hoursPerWeek}</span>
                </div>
                <div className="col-span-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-slate-650">ID Proof Document</span>
                      {selectedRecord.idProofUrl && (
                        ...
                        >
                          <FileText className="w-3 h-3" /> Download Original
                        </a>
                      )}
                    </div>
                    {selectedRecord.idProofUrl ? (
