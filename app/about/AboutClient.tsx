...
export default function AboutClient({ teamMembers }: AboutClientProps) {
  ...
  const advisors = teamMembers.filter(m => m.role.toLowerCase() !== "founder");
  ...
  return (
    <div className="flex flex-col w-full">
            ...
            {founders.map((founder, index) => {
              ...
              return (
                <motion.div
                  ...
                  key={founder.id}
                  className={`grid grid-cols-1 lg:grid-cols-12 gap-8 items-center p-8 md:p-12 rounded-[2.5rem] backdrop-blur-md border border-[#B8C5D6] bg-white/80 shadow-premium transition-colors duration-500 cursor-pointer ${hoverColors[index % hoverColors.length]} ${index % 2 === 1 ? "lg:flex-row-reverse" : ""
                    }`}
                >
                  <div className={`lg:col-span-4 relative aspect-square w-full max-w-[280px] mx-auto rounded-3xl overflow-hidden shadow-soft border-2 border-white shrink-0 ${index % 2 === 1 ? "lg:order-2" : ""
                    }`}>
                    <img
                      referrerPolicy="no-referrer"
                      src={founder.image}
            ...
            {advisors.map((adv, index) => {
              ...
              return (
                <motion.div
                  ...
                  transition={{ duration: 0.6 }}
                  key={adv.id}
                  className={`grid grid-cols-1 lg:grid-cols-12 gap-8 items-center p-8 md:p-12 rounded-[2.5rem] backdrop-blur-md border border-[#E3D38C] bg-white/80 shadow-premium transition-colors duration-500 cursor-pointer ${hoverColors[index % hoverColors.length]} ${index % 2 === 1 ? "lg:flex-row-reverse" : ""
                    }`}
                >
                  <div className={`lg:col-span-4 relative aspect-square w-full max-w-[280px] mx-auto rounded-3xl overflow-hidden shadow-soft border-2 border-white shrink-0 ${index % 2 === 1 ? "lg:order-2" : ""
                    }`}>
      ...
      {/* Basic vanilla styles for premium prose rendering */}
      <style jsx global>{`
        .prose p {
